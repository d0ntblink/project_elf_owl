...
class PythonDepandaAnalyzer:
    ...
    #     self.logger.debug(f"Analyzing dependencies in code...")
    #     tree = ast.parse(self.content)
    #     for node in ast.walk(tree):
    #             if isinstance(node, ast.Import):
    #                 for alias in node.names:
    #                     package_name = alias.name.split('.')[0].lower()  # Get the root package
    #                     if package_name not in self.dependencies:
    #                         latest_version = self._fetch_latest_version(package_name)
    #                         if latest_version:
    #                             self.dependencies= self._resolve_all_dependencies(package_name, latest_version, resolved=self.dependencies)
    #                         else:
    #                             pass
    #             elif isinstance(node, ast.ImportFrom):
    #                 if node.module:
    #                     package_name = node.module.split('.')[0].lower()  # Get the root package
    #                     if package_name not in self.dependencies:
    #                         latest_version = self._fetch_latest_version(package_name)
    #                         if latest_version:
    #                             self.dependencies = self._resolve_all_dependencies(package_name, latest_version, resolved=self.dependencies)
